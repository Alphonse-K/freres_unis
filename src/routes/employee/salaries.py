...
def create_salary(data: SalaryCreate, db: DB, current_user: CanCreateEmployee):
    return SalaryService.create(db, data)

@salary_router.get("/salaries", response_model=list[SalaryOut])
def list_salaries(db: DB, current_user: CanReadEmployee, pagination: PaginationParams = Depends()):
    return SalaryService.list(db, pagination)

@salary_router.get("/salaries/employee/{employee_id}", response_model=list[SalaryOut])
def list_salaries_by_employee(employee_id: int, db: DB, current_user: CanReadEmployee, pagination: PaginationParams = Depends()):
...
def get_salary(salary_id: int, db: DB, current_user: CanReadEmployee):
    return SalaryService.get(db, salary_id)

@salary_router.patch("/salaries/{salary_id}", response_model=SalaryOut)
def update_salary(salary_id: int, data: SalaryUpdate, db: DB, current_user: CanUpdateEmployee):
    return SalaryService.update(db, salary_id, data)

@salary_router.delete("/salaries/{salary_id}")
def delete_salary(salary_id: int, db: DB, current_user: CanDeleteEmployee):
